# Linear Regression: Stochastic Gradient Descent, Ground Up
...
def stochastic_gradient_descent(x_train, y_train):
    a = random.randint(0, len(x_train))     # Choosing a row at random
    ...
    while delta > 1:
        ...
        for i in range(len(theta)):     # Theta update: This loop calculates the partial derivative, and updates theta
            p_d_sum = 0
            h_theta_x = sum(theta[k] * x_train.iloc[a, k] for k in range(len(theta)))
            p_d_sum = p_d_sum + (h_theta_x - y_train.iloc[a]) * x_train.iloc[a, i]
...
def main():
    ...
    theta = stochastic_gradient_descent(x_train, y_train)        # Obtain thetas
    print(theta)
    y_pred = np.dot(x_test, theta)      # Predicted y values
    r2 = r2_score(y_test, y_pred)       # Calculate r squared score
    print("R^2 score of the stochastic gradient descent model:", r2)
